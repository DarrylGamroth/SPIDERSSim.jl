...
those data were converted into the scalar LLOWFS model in `SpidersProper`, and
...
| Zemax surface | Recovered element | Value used by `SpidersProper` |
| --- | --- | --- |
...
157.607199 mm at 1265 nm and 157.719631 mm at 1650 nm. `SpidersProper` computes
...
- `src/llowfs.jl`: prescription, catalog dispersion, ABCD checks, and relay
  propagation;
- `src/prepared.jl`: prepared common-path and LLOWFS propagation state;
- `test/runtests.jl`: refractive-index, first-order, energy, sampling, and
  zero-allocation checks;
- `benchmarks/benchmark_prepared_llowfs.jl`: warmed CPU benchmark;
- `benchmarks/benchmark_cuda_prepared_llowfs.jl`: CUDA benchmark entry point.
...
`CUDA.synchronize()`; the remainder was inside `llowfs_propagate!`. The CPU
