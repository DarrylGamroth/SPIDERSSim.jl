"""
    LLOWFSRelayConfiguration(; kwargs...)
...
"""
Base.@kwdef struct LLOWFSRelayConfiguration
...
function validate(configuration::LLOWFSRelayConfiguration)
    configuration.lyot_to_lens_m >= 0 || throw(ArgumentError(
        "lyot_to_lens_m must be nonnegative"))
    configuration.s_lah71_thickness_m > 0 || throw(ArgumentError(
        "s_lah71_thickness_m must be positive"))
    configuration.s_nph5_thickness_m > 0 || throw(ArgumentError(
        "s_nph5_thickness_m must be positive"))
    configuration.lens_clear_radius_m > 0 || throw(ArgumentError(
        "lens_clear_radius_m must be positive"))
    configuration.lens_to_filter_m >= 0 || throw(ArgumentError(
        "lens_to_filter_m must be nonnegative"))
    configuration.filter_thickness_m >= 0 || throw(ArgumentError(
        "filter_thickness_m must be nonnegative"))
    configuration.filter_to_detector_m >= 0 || throw(ArgumentError(
        "filter_to_detector_m must be nonnegative"))
    configuration.detector_width_m > 0 || throw(ArgumentError(
        "detector_width_m must be positive"))
    configuration.detector_height_m > 0 || throw(ArgumentError(
        "detector_height_m must be positive"))
    configuration.detector_pixel_pitch_m > 0 || throw(ArgumentError(
        "detector_pixel_pitch_m must be positive"))
    configuration.detector_pixels_x > 0 || throw(ArgumentError(
        "detector_pixels_x must be positive"))
    configuration.detector_pixels_y > 0 || throw(ArgumentError(
        "detector_pixels_y must be positive"))
    configuration.measured_roi_pixels_x > 0 || throw(ArgumentError(
        "measured_roi_pixels_x must be positive"))
    configuration.measured_roi_pixels_y > 0 || throw(ArgumentError(
        "measured_roi_pixels_y must be positive"))
    isapprox(
        configuration.detector_width_m,
        configuration.detector_pixels_x * configuration.detector_pixel_pitch_m;
        rtol=0,
        atol=eps(configuration.detector_width_m) * 8,
    ) || throw(ArgumentError(
        "detector width, pixel count, and pixel pitch are inconsistent"))
    isapprox(
        configuration.detector_height_m,
        configuration.detector_pixels_y * configuration.detector_pixel_pitch_m;
        rtol=0,
        atol=eps(configuration.detector_height_m) * 8,
    ) || throw(ArgumentError(
...
function _llowfs_lens_matrix(wavelength_m::Real, configuration::LLOWFSRelayConfiguration)
    indices = llowfs_refractive_indices(
        wavelength_m;
        temperature_c=configuration.temperature_c,
    )
    front_power = (indices.s_lah71 - 1) * configuration.front_curvature_inv_m
    cement_power = (indices.s_nph5 - indices.s_lah71) *
                   configuration.cement_curvature_inv_m
    return _multiply_abcd(
        _translation_matrix(configuration.s_nph5_thickness_m / indices.s_nph5),
        _multiply_abcd(
            _refraction_matrix(cement_power),
            _multiply_abcd(
                _translation_matrix(configuration.s_lah71_thickness_m / indices.s_lah71),
...
@inline function _cement_to_detector_reduced_m(configuration::LLOWFSRelayConfiguration, indices)
    return configuration.s_nph5_thickness_m / indices.s_nph5 +
           configuration.lens_to_filter_m +
           configuration.filter_thickness_m / indices.fused_silica +
           configuration.filter_to_detector_m
end

"""
    llowfs_relay_summary(wavelength_m; configuration=LLOWFSRelayConfiguration())
...
function llowfs_relay_summary(
    wavelength_m::Real;
    configuration::LLOWFSRelayConfiguration=LLOWFSRelayConfiguration(),
)
    validate(configuration)
    lens, indices = _llowfs_lens_matrix(wavelength_m, configuration)
    a, _, c, _ = lens
    ...
    from_lyot = _multiply_abcd(
        lens,
        _translation_matrix(configuration.lyot_to_lens_m),
    )
...
"""
struct PreparedLLOWFSPropagation{C,LC,E,CTX,W,AP,A,F,L,S,O,I} <: AbstractPreparedPropagation
    configuration::C
    relay_configuration::LC
    entrance::E
    context::CTX
    wavefront::W
    apertures::AP
...
function _prepare_llowfs_relay!(
    wf::Proper.WaveFront,
    configuration::LLOWFSRelayConfiguration,
    indices,
    context::Proper.RunContext,
)
    T = real(eltype(wf.field))
    prop_propagate(wf, T(configuration.lyot_to_lens_m), context, "LLOWFS lens front")
    front_focal_length = inv(
        T(indices.s_lah71 - 1) * T(configuration.front_curvature_inv_m),
    )
    prop_lens(wf, front_focal_length, context, "LLOWFS lens surface 1")
    lens = _prepare_clear_aperture_internal(
        wf,
        T(2 * configuration.lens_clear_radius_m),
    )
    _apply_internal_multiplier!(wf, lens)
    prop_propagate(
        wf,
        T(configuration.s_lah71_thickness_m / indices.s_lah71),
        context,
        "LLOWFS S-LAH71",
    )
    cement_focal_length = inv(
        T(indices.s_nph5 - indices.s_lah71) *
        T(configuration.cement_curvature_inv_m),
    )
    prop_lens(wf, cement_focal_length, context, "LLOWFS lens surface 2")
    prop_propagate(
        wf,
        T(_cement_to_detector_reduced_m(configuration, indices)),
        context,
...
"""
    prepare_llowfs_propagation(wavelength_m, gridsize; kwargs...)
...
[`prepare_scc_propagation`](@ref). Detector pixel integration, noise, and hardware ROI
readout remain the responsibility of AdaptiveOpticsSim and the HIL model.
"""
function prepare_llowfs_propagation(
    wavelength_m::Real,
    gridsize::Integer;
    configuration::SCCPropagationConfiguration=SCCPropagationConfiguration(),
    relay_configuration::LLOWFSRelayConfiguration=LLOWFSRelayConfiguration(),
    ...
    indices = llowfs_refractive_indices(
        wavelength_m;
        temperature_c=relay_configuration.temperature_c,
    )
    llowfs_lens = _prepare_llowfs_relay!(wf, relay_configuration, indices, context)
    ...
    prepared = PreparedLLOWFSPropagation(
        configuration,
        relay_configuration,
        entrance,
        context,
...
"""
function propagate_llowfs!(prepared::PreparedLLOWFSPropagation)
...
llowfs_intensity(prepared::PreparedLLOWFSPropagation) = prepared.output
